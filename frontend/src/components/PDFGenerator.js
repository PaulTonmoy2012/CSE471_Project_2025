...
export const prescriptionPDF = (formData) => {
  const docDefinition = {
    content: [
      ...
      {
        text: 'Thank you for choosing PAUL-HMS Health Service.',
        style: 'footer',
        alignment: 'center',
        margin: [0, 30],
      },
...
const generateBillPDF = (bills, type) => {
  const hospitalName = 'PAUL-HMS Health Service';
  ...
  const docDefinition = {
    content: [
      ...
      {
        text: 'Thank you for visiting PAUL-HMS Health Service.',
        style: 'footer'
      }
    ],
    styles: {
      ...
      ...bills.map(bill => ({
        table: {
          ...
          body: [
            ...
            ['Total Bill', `à§³ ${bill.totalBill || 'N/A'}`],
            ['Date', new Date(bill.date).toLocaleDateString() || 'N/A'],
            ['Medicines', bill.medicines.length ? bill.medicines.map(item => `${item.medicineId.name}: à§³ ${item.totalPrice} for ${item.quantity} units`).join(', ') : 'N/A'],
            ['Paid Status', {
              text: bill.paid ? 'Paid' : 'Unpaid',
              color: bill.paid ? 'green' : 'red',
              bold: true
